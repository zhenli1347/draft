...
the first declaration of the function template\iref{temp.dep.candidate}.
\end{note}
\begin{example}
\begin{codeblock}
...
\pnum
When the inject-class-name of a class template specialization or
...
shall not be bound to any following declaration contained by the scope
...
it can be deduced\iref{temp.deduct.decl}.
...
provided it can be deduced\iref{temp.deduct.decl}.
\begin{example}
\begin{codeblock}
template<class T> class Array { @\commentellip@ };
